...
export function getBookmarksMenu() {
  return {
    ...
    createItem: () => {
      ...
      const pushForUri = (menu, uri, name) => {
        ...
        if (file.query_exists(null)) {
          ...
          try {
            ...
            // It seems that the folder icons all look the same with some icon themes if
            // the icon string ends with this items. We just remove them, then they seem
            // to use the correct images...
      ...
      try {
        ...
        if (ok) {
          ...
          const string = new TextDecoder().decode(bookmarks);
          string.split(/\r?\n/).forEach(uri => {

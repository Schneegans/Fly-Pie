...
export function getRunningAppsMenu() {

  // This is used as data for newly created items of this type.
  const defaultData =
      {activeWorkspaceOnly: false, appGrouping: true, hoverPeeking: true, nameRegex: ''};
  ...
  return {
    ...
    config: {
      ...
      // parameter is a callback which must be fired whenever the user changes something
      // in the widgets.
      getWidget(data, updateCallback) {
        ...
        const _createToggle = (i, name, value) => {
          ...
          hBox.append(
              new Gtk.Label({label: name, halign: Gtk.Align.START, hexpand: true}));
        ...
        toggles[0] =
            _createToggle(0, _('Active Workspace Only'), data.activeWorkspaceOnly);
    ...
    createItem: (data) => {
      ...
      apps.forEach(app => {
        ...
        // We will add the window items directly to the result menu. Only if there are
        // more than one window for the current app and grouping is enabled, we will
        // create a submenu.

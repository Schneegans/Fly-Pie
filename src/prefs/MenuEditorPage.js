...
var MenuEditorPage = class MenuEditorPage {
  ...
  constructor(builder, settings) {
    ...
    //         // Load the configuration! We do a parse / stringify to catch any JSON
    //         errors
    //         // here.
    //         if (success) {
    ...
    //   const size  = Math.min(widget.get_allocated_width(),
    //   widget.get_allocated_height()); const icon  = this._getSelected('ICON'); const
    //   font  = this._settings.get_string('font'); const color =
    //   widget.get_style_context().get_color(); if (icon && icon.length > 0) {

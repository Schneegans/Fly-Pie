...
var TileMenu = class TileMenu {
  ...
  constructor(onSelect, onCancel) {
    ...
    let itemContainer = new St.Widget({style_class : "switcher-list"});
  ...
  destroy() { this._background.destroy(); }
  ...
  _createMenuItems(parentItem, description, path) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let background = new St.Widget({style_class : "switcher-list", opacity : 0});
      ...
      let label = new St.Label({text : labelText, opacity : 0, style : "font-size: 80%"});
      ...
      // create main button --------------------------------------------------------------
      menu.connect('enter-event', (m) => this._onItemEnter(m));
      menu.connect('leave-event', (m) => this._onItemLeave(m));
      menu.connect('button-release-event', (m, e) => this._onItemClicked(m, e));
  ...
  _getMenuWidth(menu) {
    ...
    return columns * this._theme.itemSize + (columns - 1) * this._theme.itemSpacing +
           2 * this._theme.menuPadding;
  }
  ...
  _setChildrenReactiveness(menu, reactive) {
    for (let j = menu.subMenus.length - 1; j >= 0; j--) {
      menu.subMenus[j].reactive = reactive;
    }
  }

  _updateMenuDepth(menu, open) {
    let timer = new Timer();
    let tween = (actor, scale) => {
